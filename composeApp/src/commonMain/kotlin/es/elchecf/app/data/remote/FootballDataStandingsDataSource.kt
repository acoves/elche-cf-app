...
/** FASE 8: football-data.org real, solo para [ClubTeam.PrimerEquipo]. La Copa del Rey no está en
 * el plan gratis — se sigue solo en el bracket de ejemplo
 * ([es.elchecf.app.data.mock.MockCupDataSource], sin cambios). Femenino e Ilicitano tampoco están
 * cubiertos (CLAUDE.md §12) y caen a [MockStandingsDataSource]. */
class FootballDataStandingsDataSource(
    private val client: HttpClient,
) : StandingsDataSource {
    private val mockFallback = MockStandingsDataSource()

    override suspend fun fetchStandings(
        competition: Competition,
        team: ClubTeam,
    ): AppResult<List<StandingRow>, AppError> {
        if (team != ClubTeam.PrimerEquipo) return mockFallback.fetchStandings(competition, team)
